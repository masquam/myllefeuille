...
function markupA(text){
  ...
  var isIn = false;
  var re = /\[([^\]]+)\]\(/;
  var next, next2, tmp, tmp2, tmp3;
  var linktext, linkuri;
  var cursor, cursorstring, bracketcount;
  ...
  while(1) {
    ...
    if (tmp !== null) {
      ...
      bracketcount = 1;
      while(1){
        cursorstring = str.charAt(cursor);
        if (!cursorstring) {
          tmp3 = str;
          break;
        }
    ...
    str = str.slice(tmp3.length + 1); // +1 = ')' bracket
  }
}


function markupImg(text){
  var str = text;
  var result = '';
  var isIn = false;
  var re = /\!\[([^\]]*)\]\(/;
  ...
  while(1) {
    ...
    result += "<img class='mylimg' src='" + linkuri + "' alt='" + linktext + "' />";
    str = str.slice(tmp3.length + 1); // +1 = ')' bracket
  }
}
...
function sanitizeUri(text) {
    ...
    // revert over-escaped characters in escapeAndReplaceReturn
    var uri = replaceAll(text, '&#39;', '\'');
    var uri = replaceAll(uri, '&amp;', '&');
