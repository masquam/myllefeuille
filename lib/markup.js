...
// sanitizeUri
// algorithm from https://www.ipa.go.jp/security/awareness/vendor/programmingv1/a01_02_main.html
function sanitizeUri(text) {
    ...
    uri.replace(/^([A-Za-z][A-Za-z0-9+\-.]*):/, '$1');
    scheme = RegExp.$1;
    ...
    // HTML escape
    // special = "&" | "<" | ">" | '"' | "'"
    // URI allowed characters only, so "<"ÅC">"ÅC'"' are already not exist
    uri = replaceAll(uri, '&', '&amp;');
    uri = replaceAll(uri, "'", '&#39;');
